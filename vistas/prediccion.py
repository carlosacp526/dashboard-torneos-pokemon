...
{"El modelo tiene ventaja estadística clara." if conf > 0.7 else "Stats parejas — cualquier resultado es posible."}
        ...
        with st.expander("🔬 Análisis avanzado (Dataset · Correlaciones · Distribuciones · Modelos)", expanded=False):
            ...
            with tab1:
                ...
                c1.metric("Top features", len(feature_cols))
                c3.metric("Jugadores únicos", latest_stats["Jugador"].nunique() if not latest_stats.empty else 0)
                c4.metric("Batallas pendientes", len(df_pend))
                st.info("Las cosechas completas no se guardan en el pkl para reducir tamaño. "
            ...
            with tab4:
                ...
                # compatibilidad con ambas versiones del pkl
                if "val_accuracy" in res_df2.columns:
                    res_df2 = res_df2.sort_values("val_auc", ascending=False).reset_index(drop=True)
                    disp = res_df2.copy()
                    disp["val_accuracy"] = (disp["val_accuracy"]*100).round(2).astype(str)+"%"
                    disp["val_auc"]      = disp["val_auc"].round(4).astype(str)
                    disp["cv_accuracy"]  = (disp["cv_accuracy"]*100).round(2).astype(str)+"%"
                    disp.columns = ["Modelo","Val Accuracy","Val AUC","CV Accuracy"]
                else:
                ...
                fig = go.Figure()
                acc_col = "val_accuracy" if "val_accuracy" in res_df2.columns else "accuracy"
                fig.add_trace(go.Bar(name="Val Acc",   x=res_df2["Modelo"],
                                     y=res_df2[acc_col]*100, marker_color="#3498db"))
                fig.add_trace(go.Bar(name="CV 5-fold", x=res_df2["Modelo"],
                                     y=res_df2["cv_accuracy"]*100, marker_color="#2ecc71"))
                ...
                st.markdown("#### Importancia de Top Features")
                ...
                with tfi:
                    ...
                    if hasattr(trained[mod_fi], "feature_importances_"):
                        fi = pd.DataFrame({"Feature":top_feat,"Importance":trained[mod_fi].feature_importances_})
                        ...
                        st.plotly_chart(fig, use_container_width=True)
                        st.caption("_g = stats J1 (ganador referencia) | _p = stats J2 | sufijo = ventana temporal")
                with tshap:
                    try:
                        import shap
