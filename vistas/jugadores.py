...
def generar_pdf_jugador(
    ...
    if desbloqueados is not None:
        ...
        def _draw_header_logros(cv, player_query, rar_name, rar_col_hex, pag_num, total_pag=5):
            ...
            txt(cv, f"LOGROS {rar_name.upper()}  ·  POKETUBI", 56, PH-36, size=7, col=C_SUBTEXT, font="Helvetica")
            ...
            txt(cv, f"{total_ok}/{total_all} logros  |  {round(total_ok/max(total_all,1)*100,1)}%  |  {xp_ganado:,}/{xp_total_posible:,} XP",
                PW/2, PR_Y+1.5, size=5.5, col=colors.white, font="Helvetica-Bold", anchor="center")

            # pastilla rareza actual destacada
            RAR_DEFS = [("BRONCE","#cd7f32"),("PLATA","#b0bec5"),("ORO","#f5c518"),("LEGENDARIO","#9c27b0")]
            PILL_W = 70; PILL_H = 12; PILL_GAP = 5
            ...
            for pi, (rlabel, rhex) in enumerate(RAR_DEFS):
                ...
                alpha_col = colors.HexColor(rhex)
                rrect(cv, px2, XP_Y-1, PILL_W, PILL_H, r=4, fill_col=alpha_col)
                # borde extra si es la rareza actual
                if rlabel == rar_name.upper():
                    ss(cv, colors.white); cv.setLineWidth(1.2)
                    cv.roundRect(px2, XP_Y-1, PILL_W, PILL_H, 4, stroke=1, fill=0)
                txt(cv, f"{rlabel}  {n_r}/{t_r}",
        ...
        LOGROS_GUIA = [
            ...
            # 119-122 faltaban en esta guía (desactualizada respecto de LOGROS en logros.py)
            (119,"Explorador Internacional","Social","Bronce",150,"Derrota a jugadores de 3 países distintos"),
            (120,"Viajero Frecuente","Social","Plata",400,"Derrota a jugadores de 5 países distintos"),
            (121,"Diplomático de Batalla","Social","Oro",900,"Derrota a jugadores de 10 países distintos"),
            (122,"Conquistador Global","Social","Legendario",2000,"Derrota a jugadores de 15 países distintos"),
            # 123-144: logros nuevos agregados en esta sesión (ver vistas/logros.py)
            (123,"Doble Racha","Victorias","Oro",700,"Encadena 5 derrotas seguidas y luego 5 victorias seguidas"),
            (124,"Rompe-Invictos","Victorias","Plata",350,"Corta una racha activa de 5+ victorias de un rival"),
            (125,"Caza-Gigantes","Victorias","Plata",400,"Vence a un rival con 200+ Elo de ventaja"),
            (126,"Caballo Negro","Victorias","Bronce",100,"Gana teniendo menos Elo que su rival"),
            (127,"Barrida","Victorias","Plata",300,"Gana una serie 3-0 sin ceder un juego"),
            (128,"Remontada de Serie","Victorias","Bronce",100,"Pierde el juego 1 pero gana la serie"),
            (129,"Comeback de Elo","Ranking","Oro",700,"Sube 200+ Elo en 3 meses o menos"),
            (130,"Number One","Ranking","Legendario",3000,"Llega a ser el Elo más alto de toda la comunidad en algún momento"),
            (131,"Elo de Acero","Ranking","Oro",800,"Mantiene 1300+ Elo durante 6 meses seguidos"),
            (132,"Explorador del Año","Estrategia","Plata",300,"Juega 5+ tiers distintos en un mismo año"),
            (133,"Triple Amenaza","Estrategia","Oro",700,"Gana en Singles, Dobles y VGC en el mismo mes"),
            (134,"Especialista","Estrategia","Oro",900,"80%+ WR en un tier con 10+ partidas en un mismo mes"),
            (135,"Rey del Caos","Estrategia","Plata",350,"60%+ WR en formatos random con 10+ partidas en un mismo mes"),
            (136,"Racha de Podios","Torneo","Oro",800,"Top 4 en 3 torneos consecutivos"),
            (137,"Final Jugada","Torneo","Plata",300,"Disputa la final de un torneo"),
            (138,"Doble Finalista","Torneo","Oro",800,"Llega a 2 finales de torneo en el mismo año"),
            (139,"Todo o Nada","Torneo","Plata",350,"Gana o pierde una final por el margen mínimo"),
            (140,"El Más Buscado","Social","Legendario",1600,"Fue el rival más enfrentado de la comunidad en algún mes"),
            (141,"Cara Conocida","Social","Plata",300,"Se enfrentó a 30+ rivales distintos"),
            (142,"Revancha Servida","Especial","Plata",300,"Gana tras perder 3 veces seguidas contra el mismo rival"),
            (143,"Fénix","Especial","Bronce",100,"Vuelve a jugar tras 6+ meses de inactividad"),
            (144,"Rivalidad Histórica","Especial","Oro",700,"15+ cruces totales contra un mismo rival"),
        ]
        ...
        HDR6 = 44
        rrect(cv, 0, PH-HDR6, PW, HDR6, r=0, fill_col=C_PANEL)
        sf(cv, C_ACCENT); cv.rect(0, PH-HDR6-2, PW, 2, fill=1, stroke=0)
        for lp in ["Logo.png","logo.png","LOGO.PNG"]:
            if os.path.exists(lp):
                try:
                    cv.drawImage(ImageReader(lp), MARGIN, PH-HDR6+4,
                                 height=36, width=36, preserveAspectRatio=True, mask='auto')
                except Exception: pass
                break
        txt(cv, player_query.upper(), 56, PH-22, size=16, col=C_TEXT, font="Helvetica-Bold")
        ...
        # barra progreso
        PR_X = MARGIN; PR_Y = PH-HDR6-15; PR_W = PW-MARGIN*2; PR_H = 8
        rrect(cv, PR_X, PR_Y, PR_W, PR_H, r=3, fill_col=C_PANEL2)
        prog_w = max(6, int(PR_W * total_ok / max(total_all, 1)))
        rrect(cv, PR_X, PR_Y, prog_w, PR_H, r=3, fill_col=C_GOLD)
        txt(cv, f"{total_ok} / {total_all} logros desbloqueados  |  {round(total_ok/max(total_all,1)*100,1)}%",
            PW/2, PR_Y+1.5, size=5.5, col=colors.white, font="Helvetica-Bold", anchor="center")

        # barra XP
        XP_Y6 = PR_Y - 11
        rrect(cv, PR_X, XP_Y6, PR_W, 7, r=3, fill_col=colors.HexColor("#1a1a2e"))
        xp_w6 = max(6, int(PR_W * xp_ganado / max(xp_total_posible, 1)))
        rrect(cv, PR_X, XP_Y6, xp_w6, 7, r=3, fill_col=colors.HexColor("#27ae60"))
        txt(cv, f"XP: {xp_ganado:,} / {xp_total_posible:,}  ({round(xp_ganado/max(xp_total_posible,1)*100,1)}%)",
            PW/2, XP_Y6+1.5, size=5.0, col=colors.white, font="Helvetica-Bold", anchor="center")

        # pastillas por rareza
        RAR_DEFS6 = [("BRONCE","#cd7f32"),("PLATA","#b0bec5"),("ORO","#f5c518"),("LEGENDARIO","#9c27b0")]
        PILL_W6 = 70; PILL_H6 = 13; PILL_GAP6 = 5
        pill_total_w6 = len(RAR_DEFS6)*(PILL_W6+PILL_GAP6)-PILL_GAP6
        pill_x0_6 = PW/2 - pill_total_w6/2
        for pi, (rlabel, rhex) in enumerate(RAR_DEFS6):
            n_r  = sum(1 for l in LOGROS if l['rareza'].upper()==rlabel and desbloqueados.get(l['id']))
            t_r  = sum(1 for l in LOGROS if l['rareza'].upper()==rlabel)
            xp_r = sum(l['xp'] for l in LOGROS if l['rareza'].upper()==rlabel and desbloqueados.get(l['id']))
            px2  = pill_x0_6 + pi*(PILL_W6+PILL_GAP6)
            rrect(cv, px2, XP_Y6-15, PILL_W6, PILL_H6, r=4, fill_col=colors.HexColor(rhex))
            txt(cv, f"{rlabel}  {n_r}/{t_r}  |  {xp_r:,}xp",
                px2+PILL_W6/2, XP_Y6-15+4,
                size=5.0, col=colors.white, font="Helvetica-Bold", anchor="center")

        # grid con todas las rarezas
        GRID6_TOP = PH - HDR6 - 50
        GRID6_BOT = MARGIN + 6
        GRID6_H   = GRID6_TOP - GRID6_BOT
        GRID6_W   = PW - MARGIN*2
        HEADER_SEC6 = 11

        RAREZA_ORDEN6 = ["Bronce","Plata","Oro","Legendario"]
        RAR_COL6 = {"Bronce":"#cd7f32","Plata":"#b0bec5","Oro":"#f5c518","Legendario":"#9c27b0"}
        RAR_BG6  = {"Bronce":"#1a0e00","Plata":"#0a1018","Oro":"#1a1400","Legendario":"#120018"}

        rar_groups6 = [
            (rar, sorted([l for l in LOGROS if l['rareza']==rar], key=lambda x: x['num']))
            for rar in RAREZA_ORDEN6
        ]
        heights_r6 = []
        for rar_name6, rar_list6 in rar_groups6:
            if rar_name6 == "Legendario":
                heights_r6.append(max(HEADER_SEC6 + 30, int(GRID6_H * 0.12)))
            else:
                heights_r6.append(max(HEADER_SEC6 + 50, int(GRID6_H * 0.29)))
        diff_r6 = GRID6_H - sum(heights_r6)
        heights_r6[0] += diff_r6

        cur_y6 = GRID6_TOP
        for (rar_name6, rar_list6), sec_h6 in zip(rar_groups6, heights_r6):
            sec_y6  = cur_y6 - sec_h6
            cur_y6 -= sec_h6
            rar_col6 = colors.HexColor(RAR_COL6[rar_name6])
            rar_bg6  = colors.HexColor(RAR_BG6[rar_name6])

            rrect(cv, MARGIN, sec_y6, GRID6_W, sec_h6, r=5, fill_col=rar_bg6)
            rrect(cv, MARGIN, sec_y6+sec_h6-HEADER_SEC6, GRID6_W, HEADER_SEC6,
                  r=4, fill_col=rar_col6)
            n_ok_r6 = sum(1 for l in rar_list6 if desbloqueados.get(l['id']))
            txt(cv, f"{rar_name6.upper()}   {n_ok_r6} / {len(rar_list6)}",
